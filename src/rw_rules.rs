...
coq_rewrite!("EGGTHSSSSOH";"               ?CHENIL0 = (ATSome ?A ?x) = (ATSome ?A ?y),              ?CHENIL1 = ?x " => " ?y " )];v.extend(vec![rewrite!("EGGHYPhyp_missing";"  (ATf (ATwadd a b))"<=>"(ATg b)"),
...
rewrite!("EGGHYPH0";"  (ATSome word b)"<=>"(ATSome word a)")].concat()); v}