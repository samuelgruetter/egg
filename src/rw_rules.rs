...
define_language! {
  pub enum CoqSimpleLanguage {
    "&left" = IDleft([Id; 0]),
    "&Syntax.cmd.set" = IDSyntaxDOTcmdDOTset([Id; 2]),
    ...
    "&@eq" = IDATeq([Id; 3]),
    "&Syntax.expr.literal" = IDSyntaxDOTexprDOTliteral([Id; 1]),
    "&l" = IDl([Id; 0]),
    "&mid" = IDmid([Id; 0]),
    ...
    "&right" = IDright([Id; 0]),
    "&Syntax.access_size.word" = IDSyntaxDOTaccess_sizeDOTword([Id; 0]),
    "&0" = ID0([Id; 0]),
    "&Z.of_nat" = IDZDOTof_nat([Id; 1]),
    "&string" = IDstring([Id; 0]),
    "&@word.rep" = IDATwordDOTrep([Id; 2]),
    "&Ascii.Ascii" = IDAsciiDOTAscii([Id; 8]),
    "&word" = IDword([Id; 0]),
    "&Syntax.expr.var" = IDSyntaxDOTexprDOTvar([Id; 1]),
    "&Z.le" = IDZDOTle([Id; 2]),
    "&and" = IDand([Id; 2]),
    "&Z.pow" = IDZDOTpow([Id; 2]),
    "&x" = IDx([Id; 0]),
    "&x1" = IDx1([Id; 0]),
    "&Syntax.bopname.add" = IDSyntaxDOTbopnameDOTadd([Id; 0]),
    "&x2" = IDx2([Id; 0]),
    "&@nil" = IDATnil([Id; 1]),
    "&EmptyString" = IDEmptyString([Id; 0]),
    "&4" = ID4([Id; 0]),
    "&String" = IDString([Id; 2]),
    "&Syntax.bopname.ltu" = IDSyntaxDOTbopnameDOTltu([Id; 0]),
    "&xs" = IDxs([Id; 0]),
    "&False" = IDFalse([Id; 0]),
    "&Z.lt" = IDZDOTlt([Id; 2]),
    "&not" = IDnot([Id; 1]),
    "&Syntax.cmd.cond" = IDSyntaxDOTcmdDOTcond([Id; 3]),
    "&@word.add" = IDATwordDOTadd([Id; 4]),
    "&Z" = IDZ([Id; 0]),
    "&8" = ID8([Id; 0]),
    "&locals" = IDlocals([Id; 0]),
    "&@word.sub" = IDATwordDOTsub([Id; 4]),
    "&Syntax.expr.load" = IDSyntaxDOTexprDOTload([Id; 2]),
    "&@map.put" = IDATmapDOTput([Id; 6]),
    "&@word.of_Z" = IDATwordDOTof_Z([Id; 3]),
    "&localsmap" = IDlocalsmap([Id; 0]),
    "&@cons" = IDATcons([Id; 3]),
    "&Syntax.expr.op" = IDSyntaxDOTexprDOTop([Id; 3]),
    "&True" = IDTrue([Id; 0]),
  }
}
...
pub fn symbol_metadata(name : &str) -> Option<(usize,bool)> {
  let v = vec![
    ...
    ("&right", (0,false)),
    ("&Syntax.access_size.word", (0,true)),
    ("&0", (0,true)),
    ("&Z.of_nat", (1,false)),
    ("&string", (0,false)),
    ("&@word.rep", (2,false)),
    ("&Ascii.Ascii", (8,true)),
    ("&word", (0,false)),
    ("&Syntax.expr.var", (1,true)),
    ("&Z.le", (2,false)),
    ("&and", (2,false)),
    ("&Z.pow", (2,false)),
    ("&x", (0,false)),
    ("&x1", (0,false)),
    ("&Syntax.bopname.add", (0,true)),
    ("&x2", (0,false)),
    ("&@nil", (1,true)),
    ("&EmptyString", (0,true)),
    ("&4", (0,true)),
    ("&String", (2,true)),
    ("&Syntax.bopname.ltu", (0,true)),
    ("&xs", (0,false)),
    ("&False", (0,false)),
    ("&Z.lt", (2,false)),
    ("&not", (1,false)),
    ("&Syntax.cmd.cond", (3,true)),
    ("&@word.add", (4,false)),
    ("&Z", (0,false)),
    ("&8", (0,true)),
    ("&locals", (0,false)),
    ("&@word.sub", (4,false)),
    ("&Syntax.expr.load", (2,true)),
    ("&@map.put", (6,false)),
    ("&@word.of_Z", (3,false)),
    ("&localsmap", (0,false)),
    ("&@cons", (3,true)),
    ("&Syntax.expr.op", (3,true)),
    ("&True", (0,false)),
  ];
...
pub fn run_simplifier(f_simplify : fn(&str, Vec<&str>) -> (), f_prove : fn(&str, &str, Vec<&str>) -> ()) {
  ...
  let st : &str = "(&@eq &Z(&Z.modulo(&@word.unsigned &64 &word(&@word.sub &64 &word &mid &x1))(&@word.unsigned &64 &word(&@word.of_Z &64 &word &8)))&0)";
  f_simplify(st, es);
}
