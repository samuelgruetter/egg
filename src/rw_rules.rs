...
pub fn make_rules() -> Vec<Rewrite<CoqSimpleLanguage, ()>> {
  let v  : Vec<Rewrite<CoqSimpleLanguage, ()>> = vec![
    ...
    rewrite!("H16"; "(&@aget &SModule &x &O)" => "(&Build_SModule &N &x9)"),
    rewrite!("H8"; "&x9" => "&N0"),
    ...
    rewrite!("H12"; "(&@fst ?A ?B(&mkpair ?A ?B ?f ?s))" => "?f"),
    coq_rewrite!("ExGoal"; "?$hyp0 = &True = (&and(&@eq &SModule(&Build_SModule(&option &N)(&@Some &N &arg_method))(&Build_SModule(&option &N)(&@None &N)))(&@eq &SModule ?new_st_s(&Build_SModule(&option &N)(&@Some &N &arg)))), ?$lhs = &True" => "(&@fst &Prop &SModule(&mkpair &Prop &SModule &ToProve ?new_st_s))"),
  ];
  v
}
...
pub fn run_simplifier(f_simplify : fn(&str, Vec<&str>) -> (), f_prove : fn(&str, &str, Vec<&str>) -> ()) {
  ...
  let st : &str = "(&@eq &SModule(&Build_SModule &N(&Npos &xH))(&Build_SModule &N &x9))";
  f_simplify(st, es);
}
